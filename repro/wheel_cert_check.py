...
Labels are canonicalized as the sorted tuple of nonzero span elements, where
quotient classes are represented by canonical coset representatives obtained by
reducing e_v against a row-echelon basis of row(M_beta).
"""
import itertools
...
def edge_labels(n, E, beta):
    ...
    labels = {}
    for (u, v) in E:
        span = {cls[u], cls[v], cls[u] ^ cls[v]} - {0}
        labels[(u, v)] = tuple(sorted(span))
    return labels

def pair_diagnoses(n, E, b1, b2):
    L1, L2 = edge_labels(n, E, b1), edge_labels(n, E, b2)
    seen = set()
    for e in E:
        key = (L1[e], L2[e])
...
def formulas(n):
    ...
    if n % 6 == 2:
        t = (m - 1) // 6
        return mk('X', 'ZYY' * (2 * t) + 'Z'), mk('X', 'YYZ' * (2 * t) + 'Z')
    if n % 6 == 4:
        s = m // 3
        base = 'YYZ' * s
        return mk('Y', base), mk('Y', base[1:] + base[0])
    raise ValueError(n)
...
if __name__ == '__main__':
    ...
    for nn in (6, 8, 10):
        ...
        full = (1 << S.npairs) - 1
        for _ in range(300):
            b1 = tuple(random.randrange(3) for _ in range(n))
            b2 = tuple(random.randrange(3) for _ in range(n))
            brute = (S.sep_mask(b1) | S.sep_mask(b2)) == full
            fast = pair_diagnoses(n, E, b1, b2)
            assert brute == fast, (nn, b1, b2, brute, fast)
    ...
    for n in range(8, 65, 2):
        ...
        nn, E = wheel(n)
        b1, b2 = formulas(n)
        assert pair_diagnoses(n, E, b1, b2), f'FORMULA FAILS for W_{n}'
        ok.append(n)
